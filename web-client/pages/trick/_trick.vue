  ...
  <div class="d-flex justify-center align-start">
    ...
    <div class="mx-2 sticky">
      <v-sheet class="mt-2 pa-2">
        <p class="text-h6 ma-0">Trick: {{ trick.name }}</p>
        <v-divider class="my-1"></v-divider>
        <p class="text-body-2 ma-0">{{ trick.description }}</p>
        <p class="text-body-2 ma-0">{{ trick.difficulty }}</p>
        <v-divider class="my-1"></v-divider>
        <div v-for="(rd, i) in relatedData" :key="rd.rowKey(i)">
          <div v-if="rd.data.length">
            <p class="text-subtitle-1 ma-0">{{ rd.title }}</p>
            <v-chip-group>
      ...
      return [
        {
          ...
          dataRoute: category => `/categories/${category.id}`,
        },
        {
